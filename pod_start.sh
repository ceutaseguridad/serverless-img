...
# ==============================================================================
# Script de Arranque v28 (Final - Lógica de Enlaces Verificada)
# ==============================================================================
...
echo "====================================================================="
echo "--- [MORPHEUS-STARTUP] INICIANDO CONFIGURACIÓN v28 ---"
echo "====================================================================="
...
# --- FASE 4: PREPARACIÓN DEL ENTORNO Y ENLACES (LÓGICA VERIFICADA) ---
echo "[MORPHEUS-STARTUP] FASE 4: Creando enlaces simbólicos basados en rutas verificadas..."
...
while IFS=, read -r type name url || [[ -n "$type" ]]; do
    ...
    # La ruta de origen SIEMPRE es la caché + el nombre del recurso.
    ...
    case "$type" in
        ...
        URL_AUTH)
            ...
            if [ -d "$SOURCE_PATH" ]; then # Es una carpeta como 'insightface'
                mkdir -p "$DEST_PATH"
                ln -sf "$SOURCE_PATH"/* "$DEST_PATH/"
                echo "[MORPHEUS-STARTUP]    -> Enlace Directorio URL_AUTH Creado: ${SOURCE_PATH}/* -> ${DEST_PATH}/"
            elif [ -f "$SOURCE_PATH" ]; then # Es un archivo como 'checkpoints/modelo.safetensors'
                mkdir -p "$(dirname "$DEST_PATH")"
                ln -sf "$SOURCE_PATH" "$DEST_PATH"
                echo "[MORPHEUS-STARTUP]    -> Enlace Archivo URL_AUTH Creado: ${SOURCE_PATH} -> ${DEST_PATH}"
            fi
            ;;
    esac
done < <(grep -v '^#' "$RESOURCE_FILE" | awk -F, '!seen[$1,$2]++')

echo "[MORPHEUS-STARTUP]    -> Creación de enlaces finalizada."
...
# ==============================================================================
# FASE 4.6: INSTALAR DEPENDENCIAS DE NODOS PERSONALIZADOS (MANTENIDO)
# ==============================================================================
